...
class LiveAgent:
    ...
    def __init__(self, checkpoint_path: str = DEFAULT_CHECKPOINT_PATH, cores: int = None):
        if not os.path.isfile(checkpoint_path): # Prüft, ob Modell vorhanden ist
            raise FileNotFoundError(
                f"Kein Checkpoint gefunden unter '{checkpoint_path}'. "
                "Der Live-Agent benötigt ein trainiertes Modell (best_champion.pt)."
            )

        self.engine = MCTSEngine(checkpoint_path, cores) # MCTS wird erzeugt
        logger.info(
    ...
    def select_move(self, game_state: GameState, time_limit_ms: int) -> Move:
        ...
        forced_move = self._find_forced_move(game_state) # Zwangszug finden
        if forced_move is not None:
            logger.info("Erzwungener Zug erkannt (Sieg/Block): x=%d z=%d", forced_move.x, forced_move.z)
            return forced_move

        player = game_state.player_slot + 1
        return self.engine.get_engine_move(game_state.board, player, time_limit_ms) # Zug berechnen lassen

    def close(self):
        self.engine.close()

    @staticmethod # Braucht nur GameState, nicht Agent(self) selbst
    # Potenzielle Win und Loss Züge finden --> optionaler Output
    def _find_forced_move(game_state: GameState) -> Optional[Move]:
        ...
        block_move = None
        for z in range(4): # Alle möglichen Züge werden durchgegangen
            for x in range(4):
                index = z * 4 + x
                ...
                if check_winner(own_board, own_value): # Prüfung Win Zug
                    return Move(x=x, z=z)

                if block_move is None:
                    opponent_board = game_state.board.copy()
                    apply_move(opponent_board, Move(x=x, z=z), opponent_value)
                    if check_winner(opponent_board, opponent_value): # Prüfung Loss Zug
                        block_move = Move(x=x, z=z)

        return block_move # Wird am Ende ausgegeben, weil Win Zug Vorrang hat