...
Dieses Dokument baut auf [`architecture_lvl1.md`](architecture_lvl1.md) auf und beschreibt die Implementierung der vier Hauptkomponenten. Es listet die wichtigsten Skripte pro Komponente auf und erläutert ihre Rolle im Datenfluss (Ebenen A bis F).
...
Das Shared-Modul enthält Spiellogik und mathematische Hilfsfunktionen, auf die die anderen Module als "Single Source of Truth" zugreifen.
...
* **`game_logic.py`**
    * *Funktion:* Regel-Engine für das 4x4x4 Brett.
    * *Inhalt:* Beinhaltet Funktionen wie `apply_move(board, move, player)` und `check_winner(board, player)`. Das Skript operiert auf der Datenebene B.
* **`state_encoder.py`**
    * *Funktion:* Live-Inferenz zwischen logischem Brett und dem neuronalen Netz.
...
* **`weak_engine.py`**
    * *Funktion:* Eine einfache Baseline-Engine (1-Step Heuristik).
* **`strong_engine.py`**
...
* **`play_terminal.py`**
    ...
    * *Inhalt:* Eine CLI-Schleife für Benchmarks ("Modell vs. Engine" usw.) inklusive Ausgabe von Metriken.
...
* **`neural_network/model.py`**
    * *Funktion:* Die Architektur des Modells.
...
* **`training/trainer.py` & `main_train.py`**
    * *Funktion:* Optimierung der Modell-Gewichte.
...
Das System für den Turnierbetrieb verbindet das Modell mit dem Server-Protokoll.

* **`main_live.py`**
    * *Funktion:* Der Startpunkt für den Live-Agenten.
* **`network/websocket_client.py`**
    * *Funktion:* Das Kommunikationsmodul.
    * *Inhalt:* Verwaltet Aufbau und Betrieb der Verbindung zum Spielserver. Empfängt und sendet JSON-Envelopes (**Datenebenen A und F**).
...
* **`agent/live_agent.py`**
    * *Funktion:* Koordinationskomponente während des Spiels.
    * *Inhalt:* Koordiniert den Datenfluss. Führt das `GameState` (Ebene B) über die Netz-Inferenz (Ebene C -> D) und wendet Heuristiken (Forced Moves) an, um unter Zeitlimit die Zugentscheidung (**Ebene E**) zu berechnen.
